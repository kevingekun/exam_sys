...
public class DateTools extends DateUtils {
		...
		public static int jianYueJinNian(Date curDate, Date oldDate)
		  ...
		  {
		    private static Map<String, TokenField> fieldTokens = new HashMap();
		    ...
		    static Date wrapDate(Date date, String fieldToken, String wrapValue) {
		      TokenField tokenField = (TokenField)fieldTokens.get(fieldToken);
		      if (tokenField == null)
		      ...
		      if (add) {
		        calendar.add(TokenField.access$0(tokenField), Integer.parseInt(wrapValue));
		      }
		      else
		        calendar.set(TokenField.access$0(tokenField), Integer.parseInt(wrapValue));
